...
import { Logging } from '@shared/models';
...
describe('[providers] - LoggerProvider', () => {
  ...
  describe('[enabled]', () => {
    beforeEach(() =>
      (context.config.get as jest.Mock).mockReturnValue(Logging.ENABLED),
    );
  ...
  describe('[disabled]', () => {
    beforeEach(() =>
      (context.config.get as jest.Mock).mockReturnValue(Logging.DISABLED),
    );
